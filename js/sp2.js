    ...
    canvas.addEventListener('dblclick', function (e) {
        ...
        dataPool.forEach(function (value) {
            ...
            if (Math.round(Area) === Math.round(Area1 + Area2 + Area3)) {
                ...
                clearCanvas();
                dataPool.forEach(function (value2) {
                    reDrawRectangles(value2[0][0], value2[0][1], value2[4], value2[3]);
                });
                return true;
            }
        });
...
//movement listener
    canvas.addEventListener('mousemove', function (e) {
        ...
        if (isActive && flag) {
            ...
            reDrawRectangles(startCoordinates.x, startCoordinates.y, calculateLineDistance(startCoordinates.x, startCoordinates.y, endCoordinates.x, endCoordinates.y), randomRGB);
            dataPool.forEach(function (value) {
                reDrawRectangles(value[0][0], value[0][1], value[4], value[3]);
            });
        } else if (isActive) {
            ...
            item[2][0] += difX;
            item[2][1] += difY;
            reDrawRectangles(item[0][0], item[0][1], item[4], item[3]);
            dataPool.forEach(function (value) {
...
//click-release listener
    canvas.addEventListener('mouseup', function (e) {
        ...
        if (!flag) {
            ...
            endCoordinates = mousePos;
         drawRectangle(1, startCoordinates.x, startCoordinates.y, endCoordinates.x, endCoordinates.y);
        }
...
// function to get current mouse position
    ...
    function reDrawRectangles(x1, y1, distance, color) {
        ...
        context.lineTo(x1 + distance / 2, y1);
    ...
    function drawRectangle(mode, x1, y1, x2, y2) {
